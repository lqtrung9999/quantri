...
const roles = ['admin', 'accountant', 'sale', 'warehouse_cn', 'customs_declaration', 'manager', 'truck_planner', 'cn_operations'];
...
if (required.some(key => !args[key]) || !roles.includes(args.role) || (args.role === 'sale' && !args.sale)) {
  console.error('Dùng: npm run add-user -- --username <tên> --name "Họ tên" --role admin|accountant|sale|warehouse_cn|customs_declaration|manager|truck_planner|cn_operations --password "Mật khẩu" [--sale "Tên Sale đúng như Google Sheet"]');
  process.exit(1);
}
