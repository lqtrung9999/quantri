(() => {
  ...
  modal.innerHTML = `<div class="crm-box"><div class="crm-head"><span>Báo cáo hoạt động Sale</span><button class="crm-close" aria-label="Đóng">×</button></div><div class="crm-report-body"><label class="crm-report-date">Chọn ngày <input id="crm-report-date" type="date"></label><div id="crm-report-content"></div></div><div class="crm-foot"><button class="crm-cancel">Đóng</button></div></div>`;
  ...
  const render = () => {
    const rows = [...document.querySelectorAll('#rows tr')].filter(row => rowDate(row) === dateInput.value);
  ...
  button.onclick = () => { dateInput.value = localIso(new Date()); render(); modal.classList.add('open'); };
