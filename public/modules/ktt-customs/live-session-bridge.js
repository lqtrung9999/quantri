(() => {
  ...
  const mapRow = row => {
    ...
    return {
      ...
      discussions: (Array.isArray(row.discussions) ? row.discussions : []).map(item => ({ id: item.id || '', actorId: item.actorId || '', actor: item.actor || '', actorRole: item.actorRole || '', content: item.content || '', priority: item.priority || 'normal', recipients: Array.isArray(item.recipients) ? item.recipients : [], attachments: Array.isArray(item.attachments) ? item.attachments : [], readBy: Array.isArray(item.readBy) ? item.readBy : [], createdAt: item.createdAt || '' })),
      discussionUnread: Number(row.discussionUnread || 0), discussionUrgentUnread: Number(row.discussionUrgentUnread || 0),
      discussionArchived: Boolean(row.discussionArchivedBy?.[currentUser?.id]),
