(() => {
    ...
    <div class="cf-dialog cf-import-dialog" role="dialog" aria-modal="true" aria-labelledby="cf-import-title">
      ...
      <div class="cf-import-body">
        ...
        <div class="cf-import-section"><div class="cf-import-section-head"><div><h3>1. Dán dữ liệu</h3><p>Có thể dán kèm hoặc không kèm dòng tiêu đề.</p></div><button id="cf-import-clear" class="cf-action" type="button">Xóa bảng dán</button></div>
          <div class="cf-import-columns">1. Ngày hàng vào kho <i></i> <em>2. Ngày bốc (tự bỏ qua)</em> <i></i> <em>3. Ngày trả khách (tự bỏ qua)</em> <i></i> 4. Mã hàng <i></i> 5. Số kiện <i></i> 6. Tên hàng <i></i> 7. Mã KH <i></i> 8. Chủ hàng <i></i> 9. Sale <i></i> 10. Phòng Sale <i></i> 11. Kế toán <i></i> 12. KG <i></i> 13. M³</div>
  ...
  const parse = () => {
    ...
    const dataRows = (header ? values.slice(1) : values).filter(line => line.some(Boolean));
    const pastedDate = value => !value || /^\d{1,4}[/-]\d{1,2}[/-]\d{1,4}$/.test(value);
    const extraDates = Boolean(values[0]?.some(cell => /ngày\s*bốc|ngày\s*trả/i.test(cell))) || dataRows.some(line => line.length >= 13 && pastedDate(line[1]) && pastedDate(line[2]));
    const offset = extraDates ? 2 : 0;
    parsedRows = dataRows.map((line, index) => ({
      line: index + 1, operationDate: displayDate(line[0]), cargoCode: line[1 + offset] || '', packageCount: line[2 + offset] || '', productName: line[3 + offset] || '', customerCode: line[4 + offset] || '', ownerName: line[5 + offset] || '', saleOwner: line[6 + offset] || '', saleTeam: line[7 + offset] || '', accountant: line[8 + offset] || '', weightKg: line[9 + offset] || '', volumeM3: line[10 + offset] || '', columnCount: line.length - offset
    }));
    renderPreview();
  };
  const renderPreview = () => {
