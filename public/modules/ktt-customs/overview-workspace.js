(() => {
  ...
  function render() {
    ...
    workspace.querySelector('.ov-all-list').innerHTML = items.length ? `<div class="ov-table-wrap"><table><thead><tr><th>Mã hàng</th><th>Số kiện</th><th>Tên hàng</th><th>Mã KH / Chủ hàng</th><th>Sale</th><th>KG</th><th>M³</th><th>Thông tin Sale</th><th>List khai báo</th><th>Xác nhận khách</th><th>Xếp xe</th><th>Trao đổi</th></tr></thead><tbody>${items.map(item => `<tr><td><b>${esc(item.code)}</b></td><td>${number(item.packs)}</td><td><b>${esc(item.name)}</b><small>${number(item.photos)} ảnh thực tế</small></td><td><b>${esc(item.customer)}</b><small>${esc(item.owner || '')}</small></td><td>${esc(item.sale || item.team || '—')}</td><td>${number(item.kg).toLocaleString('vi-VN')}</td><td>${number(item.m3).toLocaleString('vi-VN',{maximumFractionDigits:2})}</td><td>${saleState(item)}</td><td>${listState(item)}</td><td>${customerState(item)}</td><td><strong class="${state(item._status)}">${label(item._status)}</strong></td><td><button type="button" class="ov-discussion" data-discussion-id="${esc(item._id)}">▢ ${item.discussions?.length || 0}</button></td></tr>`).join('')}</tbody></table></div>` : '<div class="ov-empty">Không có mã hàng phù hợp.</div>';
  }
  ...
  discussionStyle.textContent = `#cf-overview-workspace .ov-discussion{border:1px solid #cfdae8;border-radius:7px;background:#fff;color:#426b9f;padding:6px 8px;font:inherit;font-size:11px;font-weight:800;cursor:pointer;white-space:nowrap}`;
