(() => {
  ...
  panel.innerHTML = `<div class="sd-dialog" role="dialog" aria-modal="true" aria-labelledby="sd-title"><header><div><p>TRAO ĐỔI NỘI BỘ THEO MÃ HÀNG</p><h2 id="sd-title"></h2><span class="sd-subtitle"></span></div><button type="button" class="sd-close" aria-label="Đóng">×</button></header><div class="sd-guidance">Nội dung được chia sẻ cho các bộ phận có quyền xem mã hàng này: Điều vận, Sale, Khai báo HQ và Quản lý.</div><div class="sd-thread"></div><form class="sd-compose"><label for="sd-message">Nội dung trao đổi</label><textarea id="sd-message" maxlength="4000" placeholder="Nêu vấn đề, thông tin cần xác nhận hoặc quyết định cần chốt..."></textarea><div><small>Trao đổi được lưu theo riêng từng mã hàng.</small><button type="submit" class="cf-action primary">Gửi trao đổi</button></div></form></div>`;
  ...
  panel.querySelector('.sd-compose').addEventListener('submit', async event => {
    event.preventDefault();
    const input = panel.querySelector('#sd-message'), content = input.value.trim();
    if (!content || !currentId) return;
    const submit = panel.querySelector('[type="submit"]'); submit.disabled = true;
    try {
