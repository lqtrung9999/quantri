(() => {
  ...
  function input(field, value, editable, scope, rowIndex) {
    const longText = field === 'description' || field === 'vi';
    if (longText) return `<textarea class="long-text ${field === 'vi' ? 'declaration-text' : 'sale-text'}" rows="3" data-${scope}-field="${field}" data-row="${rowIndex}" ${editable ? '' : 'disabled'}>${esc(value)}</textarea>`;
