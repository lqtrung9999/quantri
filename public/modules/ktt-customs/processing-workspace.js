(() => {
  ...
  const imageFor = source => new Promise((resolve, reject) => { const image = new Image(); image.onload = () => resolve(image); image.onerror = reject; image.src = source; });
  ...
  async function downloadWorkspaceConfirmation(item) {
    ...
    try { const logo = await imageFor('/logo-kim-thanh-tin-transparent.png'); const maxWidth = 210, maxHeight = 88, ratio = Math.min(maxWidth / logo.width, maxHeight / logo.height); const width = logo.width * ratio, imageHeight = logo.height * ratio; context.drawImage(logo, tableWidth + padding - width, 14, width, imageHeight); } catch {}
    let x = padding, y = top; context.fillStyle = '#ddd1f5'; context.fillRect(x, y, tableWidth, headerHeight); context.strokeStyle = '#b9afd5';
    confirmationFields.forEach(([label], column) => { context.strokeRect(x, y, widths[column], headerHeight); context.fillStyle = '#30294a'; context.font = '700 14px Arial'; canvasLines(context, label, widths[column] - 16, 3).forEach((text, line) => context.fillText(text, x + 8, y + 24 + line * 18)); x += widths[column]; }); y += headerHeight;
    layout.forEach((row, rowIndex) => { x = padding; row.values.forEach((value, column) => { context.fillStyle = rowIndex % 2 ? '#fbfaf7' : '#fff'; context.fillRect(x, y, widths[column], row.height); context.strokeStyle = '#d9dfeb'; context.strokeRect(x, y, widths[column], row.height); context.fillStyle = '#1d2636'; context.font = '15px Arial'; canvasLines(context, value, widths[column] - 18).forEach((text, line) => context.fillText(text, x + 9, y + 23 + line * 22)); x += widths[column]; }); y += row.height; });
  ...
  style.textContent = `
    ...
    .xp-confirm-title{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 16px}.xp-confirm-title>div{display:grid;gap:5px}.xp-confirm-title b{font-size:16px}.xp-confirm-title span{color:#66758d}.xp-confirm-wrap{margin:0 16px 5px;overflow:auto;border:1px solid #dce4ef;border-radius:9px}.xp-confirm-wrap table{border-collapse:collapse;min-width:1700px;width:100%;font-size:11px}.xp-confirm-wrap th{padding:10px 9px;background:#ddd1f5;color:#30294a;text-align:left;white-space:normal;border-right:1px solid #bfb4dc}.xp-confirm-wrap td{padding:11px 9px;border-top:1px solid #dfe5ee;border-right:1px solid #e2e7ef;vertical-align:top}.xp-confirm-wrap th:nth-child(3),.xp-confirm-wrap td:nth-child(3){min-width:330px;white-space:normal}.xp-confirm-card .xp-actions{border-top:1px solid #e3e9f2}
  `;
