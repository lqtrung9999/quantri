(() => {
  ...
  workspace.innerHTML = `
    <div class="xp-head"><div><h1>Xử Lý Khai Báo</h1><p>Bộ phận Khai báo xử lý list từ thông tin Sale đã gửi. Các cột nhận diện được giữ cố định khi cuộn ngang.</p></div><button id="xp-back" class="cf-action">← Danh sách công việc</button></div><div class="xp-rate"></div><div class="xp-kpis"></div>
  ...
  function lineImages(line) {
    const raw = Array.isArray(line?.images) ? line.images : (line?.image ? [{ url: line.image }] : []);
    return raw.filter(image => image?.url).map(image => ({ id: String(image.id || ''), url: String(image.url), fileName: String(image.fileName || ''), mimeType: String(image.mimeType || '') }));
  }
  ...
  function extraInfo(fields) {
    ...
    return items.length ? `<div class="xp-extra-info">${items.map(field => `<div><b>${esc(field.label)}</b><span>${esc(field.value)}</span></div>`).join('')}</div>` : '<span class="xp-extra-empty">—</span>';
  }
  function shipmentCard(item) {
    const user = session().user;
    // Sale now has a dedicated workspace. This screen intentionally keeps
    // Sale data read-only so declaration work and Sale entry do not collide.
    const saleEditable = false;
    ...
    return `<article class="xp-card" data-id="${esc(item._id)}" data-code="${esc(item.code)}">
      ...
      <div class="xp-table-wrap"><table class="xp-table"><thead><tr><th class="pin code" rowspan="2">Mã hàng</th><th class="pin product" rowspan="2">Tên hàng Kho TQ</th><th class="pin image" rowspan="2">Ảnh hàng</th><th rowspan="2" class="sale-extra-head">Thông tin thêm Sale</th><th colspan="${saleFields.length}" class="sale-group">THÔNG TIN SALE</th><th colspan="${customsFields.length}" class="customs-group">LIST KHAI BÁO</th><th class="xp-operation-head" rowspan="2">Thao tác</th></tr><tr>${saleFields.map(([, label]) => `<th class="sale-head">${esc(label)}</th>`).join('')}${customsFields.map(([, label]) => `<th class="customs-head">${esc(label)}</th>`).join('')}</tr></thead><tbody>
      ${lines.map((line, index) => { const images = lineImages(line.sale); return `<tr data-sale-images="${esc(JSON.stringify(images))}" data-sale-extras="${esc(JSON.stringify(line.sale.extraFields || []))}"><td class="pin code"><b>${esc(item.code)}</b><small>Dòng ${index + 1}</small></td><td class="pin product">${esc(item.name)}</td><td class="pin image">${imageCell(images, saleEditable)}</td><td>${extraInfo(line.sale.extraFields)}</td>${saleFields.map(([field]) => `<td>${input(field, line.sale[field] ?? '', saleEditable, 'sale', index)}</td>`).join('')}${customsFields.map(([field, , type]) => { const suggested = field === 'invoicePrice' ? (line.customs[field] || line.sale.invoicePrice || '') : field === 'qty1' ? (line.customs[field] || line.sale.qty || '') : field === 'unit1' ? (line.customs[field] || line.sale.unit || 'Cái') : line.customs[field] ?? ''; return `<td>${input(field, suggested, customsEditable && type !== 'readonly', 'customs', index, field === 'price' && line.customs.priceManual)}</td>`; }).join('')}<td class="xp-line-actions">${saleEditable || customsEditable ? '<button type="button" class="xp-clone-line">Nhân bản</button><button type="button" class="xp-delete-line">Xóa dòng</button>' : '—'}</td></tr>`; }).join('')}
      </tbody></table></div>
  ...
  function salePayload(card) {
    return collect(card, 'sale', saleFields).map((line, index) => {
      const row = card.querySelectorAll('.xp-table tbody tr')[index]; let images = [];
      try { images = JSON.parse(row?.dataset.saleImages || '[]'); } catch { images = []; }
      let extraFields = []; try { extraFields = JSON.parse(row?.dataset.saleExtras || '[]'); } catch { extraFields = []; }
      return { description: line.description, packageCount: line.packs, productsPerPackage: line.productsPerPack, productSize: line.size, declarationQuantity: line.qty, declarationUnit: line.unit, invoicePriceBeforeVat: line.invoicePrice, note: line.note, extraFields, images };
    });
  }
  ...
  function clearCardRows(card) {
    ...
    if (all[0]) { all[0].dataset.saleImages = '[]'; const cell = all[0].querySelector('.xp-image-cell'); if (cell) cell.outerHTML = imageCell([], true); }
  }
  ...
  workspace.addEventListener('click', event => {
    ...
    const removeImage = event.target.closest('.xp-image-remove');
    if (removeImage) { const row = removeImage.closest('tr'), images = parseRowImages(row); images.splice(Number(removeImage.dataset.imageIndex), 1); row.dataset.saleImages = JSON.stringify(images); updateImageCell(row); workspace.dataset.dirty = '1'; return; }
  ...
  workspace.addEventListener('change', event => {
    const imageInput = event.target.closest('.xp-sale-image-input');
    if (imageInput?.files?.length) { uploadSaleImages(imageInput.closest('.xp-card'), imageInput.closest('tr'), imageInput.files); return; }
  ...
  style.textContent = `
    ...
    .xp-table-wrap{max-width:100%;overflow:auto;border-bottom:1px solid #e0e7f0;scrollbar-gutter:stable}.xp-table{border-collapse:separate!important;border-spacing:0;min-width:3600px!important;width:max-content!important;font-size:11px!important}.xp-table th,.xp-table td{height:auto!important;padding:5px!important;border-right:1px solid #dfe6ef;border-top:1px solid #e7ecf3;background:#fff;vertical-align:top}.xp-table thead th{position:sticky;top:0;z-index:3;min-width:108px;padding:7px 6px!important;white-space:normal;font-size:11px!important}.xp-table .sale-group,.xp-table .sale-head,.xp-table .sale-extra-head{background:#e8f4d9!important;color:#2c4826}.xp-table .customs-group,.xp-table .customs-head{background:#eee7fb!important;color:#4e3a78}.xp-table .pin{position:sticky;z-index:4;background:#f8fafc!important}.xp-table .pin.code{left:0;min-width:112px;width:112px}.xp-table .pin.product{left:112px;min-width:150px;width:150px}.xp-table .pin.image{left:262px;min-width:125px;width:125px;box-shadow:5px 0 10px #263b5b16}.xp-table thead .pin{z-index:6}.xp-table td.pin small{display:block;margin-top:4px;color:#7b889b}.xp-extra-info{display:grid;gap:4px;min-width:175px;max-width:220px}.xp-extra-info div{display:grid;gap:2px;padding:4px 0;border-bottom:1px solid #edf1f5}.xp-extra-info b{color:#567044;font-size:10px}.xp-extra-info span{white-space:pre-wrap}.xp-extra-empty{color:#8995a7}.xp-table input,.xp-table textarea,.xp-table .xp-unit-select{box-sizing:border-box;width:108px;min-width:108px;height:32px;border:1px solid #cfd9e7;border-radius:6px;padding:5px 6px;font:11px/1.35 system-ui;background:#fff;color:#172033}.xp-table textarea.long-text{height:72px;resize:vertical;white-space:pre-wrap}.xp-table textarea.sale-text{width:360px;min-width:360px}.xp-table textarea.english-text{width:215px;min-width:215px;height:80px}.xp-table textarea.declaration-text{width:430px;min-width:430px;height:80px}.xp-table input:disabled,.xp-table textarea:disabled,.xp-table .xp-unit-select:disabled{border-color:transparent;background:#f3f6fa;color:#536178;opacity:1}.xp-actions{padding:9px 13px}.xp-actions>span{color:#68768c;font-size:11px}.xp-actions>div{display:flex;gap:7px}.xp-actions .cf-action,.xp-tools .cf-action,.xp-tools select{font-size:11px!important;height:34px}.xp-empty{padding:25px;text-align:center;background:#fff;border:1px solid #dce4ef;border-radius:12px;color:#738098}@media(max-width:900px){#cf-processing-workspace{padding:12px}.xp-head{align-items:flex-start}.xp-head p{max-width:620px}.xp-card-head{align-items:flex-start;flex-direction:column}}
    .xp-image-cell{display:grid;gap:5px;min-width:115px}.xp-image-list{display:flex;flex-wrap:wrap;justify-content:center;gap:4px;min-height:30px}.xp-image-thumb{display:block;width:35px;height:35px;border:1px solid #c8d4e3;border-radius:5px;overflow:hidden;background:#fff}.xp-image-thumb img{width:100%;height:100%;object-fit:cover}.xp-image-remove{align-self:flex-start;margin-left:-7px;margin-top:-5px;width:16px;height:16px;padding:0;border:1px solid #de9b9b;border-radius:50%;background:#fff;color:#b53232;font-size:13px;line-height:12px;cursor:pointer}.xp-image-empty{align-self:center;color:#78869b;font-size:10px}.xp-image-upload{display:block;border:1px dashed #80a16b;border-radius:6px;padding:5px 4px;background:#f5fbef;color:#3d6f2c;font-size:10px;font-weight:800;cursor:pointer}.xp-image-upload input{display:none!important}.xp-image-cell small{color:#7a8798;font-size:9px;line-height:1.25}
    ...
    .xp-table th{text-align:center!important;vertical-align:middle!important;font-weight:800!important}.xp-table td{text-align:center!important;vertical-align:middle!important}.xp-table .pin.product{text-align:left!important;vertical-align:middle!important}.xp-table input,.xp-table textarea,.xp-table .xp-unit-select{box-sizing:border-box!important;width:100%!important;min-width:0!important;height:80px!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;outline:0!important;padding:6px 1px!important;text-align:center;vertical-align:middle}.xp-table td:focus-within{background:#fff9df!important}.xp-table textarea.long-text{height:80px!important;min-height:80px!important;line-height:16px!important;overflow:auto}.xp-table textarea[data-sale-field="description"],.xp-table textarea[data-customs-field="en"],.xp-table textarea[data-customs-field="vi"],.xp-table input[data-sale-field="note"],.xp-table input[data-customs-field="note"]{text-align:left!important}.xp-table input:disabled,.xp-table textarea:disabled,.xp-table .xp-unit-select:disabled{background:transparent!important;color:#536178!important;opacity:1}.xp-table input:not(:disabled),.xp-table textarea:not(:disabled),.xp-table select:not(:disabled){background:transparent!important;color:#172033!important}
