(() => {
  ...
  workspace.innerHTML = `<div class="tl-head"><div><h1>Xếp Xe CN</h1><p>Sale gửi thông tin cho Khai báo là mã hàng có thể được xếp xe; không cần chờ khách xác nhận.</p></div><button class="cf-action tl-back">← Khai báo &amp; xếp xe</button></div>
    <div class="tl-kpis"><article><span>MÃ CHỜ XẾP</span><b id="tl-ready-count">0</b><small>Đang còn hàng</small></article><article><span>KIỆN CHỜ XẾP</span><b id="tl-ready-packs">0</b><small>Tổng số kiện còn lại</small></article><article><span>KHỐI CHỜ XẾP</span><b id="tl-ready-volume">0 m³</b><small>Căn gọi xe Trung Quốc</small></article><article><span>ĐANG XẾP MỘT PHẦN</span><b id="tl-partial-count">0</b><small>Còn hàng cho chuyến sau</small></article><article><span>ĐÃ XONG LIST KHAI BÁO</span><b id="tl-customs-done-count">0</b><small>Đã gửi list khai báo</small></article><article><span>ĐÃ XÁC NHẬN KHÁCH</span><b id="tl-customer-approved-count">0</b><small>Có thể đối chiếu xe</small></article><article><span>CHƯA XÁC NHẬN KHÁCH</span><b id="tl-customer-pending-count">0</b><small>Đang chờ khách phản hồi</small></article></div>
    ...
    <div class="tl-table-wrap"><table><thead><tr><th>Chọn</th><th>Mã hàng</th><th>Tên hàng</th><th>Mã KH / Chủ hàng</th><th>Sale / Phòng</th><th>Ngày kho</th><th>Tổng kiện</th><th>Tổng m³</th><th>Còn kiện</th><th>Còn m³</th><th>Trạng thái</th><th>Trao đổi</th><th>Xe đã bốc</th></tr></thead><tbody id="tl-body"></tbody></table></div>
  ...
  function render() {
    ...
    const customsDone = rows().filter(row => ['customer_confirmation', 'ready_for_loading', 'loaded'].includes(row._status));
    const customerApproved = rows().filter(row => ['ready_for_loading', 'loaded'].includes(row._status));
    const customerPending = rows().filter(row => row._status === 'customer_confirmation');
