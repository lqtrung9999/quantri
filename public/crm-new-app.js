(() => {
  ...
  const select = (values, current, editable, field) => `<select class="status" data-field="${field}" ${editable ? '' : 'disabled'}>${values.map(value => `<option value="${esc(value)}" ${value === current ? 'selected' : ''}>${esc(value)}</option>`).join('')}</select>`;
  ...
  $('#rows').addEventListener('change', event => { const selectElement = event.target.closest('select[data-field]'); if (!selectElement) return; const row = selectElement.closest('tr'), lead = leads.find(item => item.id === row.dataset.id); if (!lead) return; const patch = { status: lead.status || '', category: lead.category || '', [selectElement.dataset.field]: selectElement.value }; updateLead(lead.id, patch).catch(error => alert(error.message)); });
